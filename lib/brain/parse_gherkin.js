...
var parseEnvFile = exports.parseEnvFile = function(callback) {
  if (__peanut__.envExists) {
    try {
      var env = __peanut__.env = require(process.cwd() +
...
function setupTestServer (env, callback) {
  ...
  var executable = "node"
  if (env.file.match("coffee")) {
    executable = "coffee"
  }
  if (env.options == undefined || env.options == null) {
    env.options = []
  }
  var testServer = spawn(executable, [env.file].concat(env.options), { cwd: process.cwd() });
...
var parseStepDefinitions = exports.parseStepDefinitions = function() {
  try {
    _(findit.findSync('features')).each(function(file) {
      var match = null;
      if ((match = file.match(/\_steps.(js|coffee)$/)) !== null) {
        if (match[1] == "coffee") { require("coffee-script") }
